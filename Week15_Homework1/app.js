...
var xss = require('xss'); //引入xss模板 防御xss攻击
...
app.post('/queryNews', function(req, res) {
    ...
    var md5 = crypto.createHash('md5'); //引入md5
    md5.update(keyword);
    var tokenServe = md5.digest('hex');
...
app.post('/editNews', function(req, res) {
    ...
    console.log(tokenServe);
    console.log(req.body.token);
    if (tokenServe != token) {
        res.json({ error: 'error', mes: 'no right' });
    }
